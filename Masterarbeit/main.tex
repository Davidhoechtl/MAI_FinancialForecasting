...
\begin{itemize}
    \item \textbf{Financial News Headlines Data} (Pham, \cite{pham_financial_news}): A Kaggle-hosted collection of financial news headlines, suitable for sentiment analysis and headline-based classification tasks.
    
    \item \textbf{Daily Financial News for 6000+ Stocks} (Aenlle, \cite{aenlle_financial_news}): A large-scale compilation of stock market news data scraped from Benzinga, covering the period from 2009 to 2020.
    
    \item \textbf{FNSPID: Financial News Dataset in Time Series} (Dong et al., \cite{dong2024fnspid}): A comprehensive dataset specifically engineered for time-series analysis, integrating stock price movements with aligned financial news records.
    
    \item \textbf{NIFTY: News-Informed Financial Trend Yield} (Saqur, \cite{raeidsaqur2024NiftyLM}): A dataset designed for aligning Large Language Models (LLMs) with financial market data, providing news context and associated stock trend labels.
\end{itemize}
