...
Measures volatility over a specific time period. \\
\hline
\textbf{Williams \%R} & 
...
\hline
Week 3--4 (Sept) & Create a simple sentiment pipeline that classifies news headlines. Analyse different finance APIs that provide daily prices for finacial products & Done \\
\hline
Week 5--6 (Oct) & Use the gathered and prepared data to train ARMA Models and compare the effect of adding the sentiment feature. (first prototype) & Done \\
\hline
...
\begin{itemize}
    ...
    \item d => degree of differencing, makes changes stationary (from price movement to price change)
...
\section{Lookahead BIAS}
After the ARMA experiment, the next step was to use more capable sentiment models than the vader lexicon. The expected outcome with FinBERT was a better performance, because the representation of the sentiment score is closer to the ground truth. The results were suprisingly good, too good in fact. This lead to some scepsis about the results. After investigating the codebase, a data leak was indentified in the training process of the ARMA model. The model had access to the headlines of tomorrow which lead to an unfair edge. While this finding changed the optimism of the use of a sentiment feature as a usefull predictor, it provided evidence that a better sentiment model leads to a better representation of the market sentiment. The positive correlation of tomorrow sentiment and tomorrow price change increased which means that the FINBERT model captchered better and more usefull information of the news headlines. (Positive correlation went from 0.0220 to 0.0283, with the AIC score decreasing by additional 21.96 points)

\section{Feature Matrix Pipeline}
To validate the results of the experiment across multiple datasets and sentiment models, a pipeline for the feature matrix was build. Following figure shows the top level architecture.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{PICs/Pipeline Architecture.png}
    \caption{Feature Matrix Pipeline - Architecture}
    \label{fig:myimage5}
\end{figure}

\subsection{Sentiment Analyzer}
This part of the pipeline is used to take an arbitrary number of financial news datasets unify, concatenate and analyze them and output an aggregated sentiment score for each date. The modular design allows to perform experiments on a variety of different datasets and sentiment models. Resulting in an efficient way to test this works hypothesis on sentiment scores.

To achieve the concatenation part of the pipeline. Each dataset has its own adapter, for transform rows in a standard format. The standard format looks like this: source;date;headline. After deduplication a VADER, FinBERT or similar models are used to create the sentiment score. And lastly those scores are grouped by granuality (week, day) and mapped to a timeseries.

% The API of the SentimentLoader looks like this:

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.5\textwidth]{PICs/SentimentLoaderAPI.png}
%     \caption{Sentiment Loader API}
%     \label{fig:myimage6}
% \end{figure}

\subsection{Impact Score Analyzer}
Many available (financial) news headline datasets have a lot of noise. For example, headlines about crimes, financial quizzes or news about foreign economics do not have the same importance than fed reserve news or US politics. Therefore in this work a novel way of denoising is introduced. An LLM is prompted to extract the importance of a given headline for the s\&p500 next day price change prediction. Similar to the sentiment analyzer the output is a panda series of impact scores, ranging von 0 (not important) to 1 (very important). After extracting the importance of a headline. The impact score is then multiplyed with the sentiment score resulting in denoised weighted sentiment score.

\subsection{Price \& Technical Indicators}
For the technical indicators and price change data, this work used investpy. Specifically, the investpy.etfs.get\_etf\_historical\_data endpoint. The minimal granuality of investpy is daily. Because of this limitation the minimal granuality of the final feature matrix cannot be intra day (hourly).
After scraping the closing prices of 'SPDR S\&P 500', price change data and technical indicators like (Volume, volatility) are calculated and added to the final feature matrix.

Following API is used as a starting point for each experiment:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{PICs/FeatureMatrixAPI.png}
    \caption{Feature Matrix API}
    \label{fig:myimage6}
\end{figure}

\section{Findings}
\subsection{Negative Correlation}
Across the datasets that has been used in the experiments (NIFTY, FNSPID, LucasPhamFinDataset). A similar trend has been indentified. Between sentiment score of today and price change of tomorrow is a \textbf{negativ correlation}. This correlation was measured via Pearson and Spearman correlation coefficient. This hints that the efficient market hypothesis holds for the 'SPDR S\&P 500' etf. Information of headlines are included in the price within <24 hours, making future predictions less effective or useless.
One possible explaination could be that investors already taking profits of the good news or using a "buy the dip" strategy for when the sentiment is negative.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{PICs/pointCloudSentimentPriceChange.png}
    \caption{Point cloud of sentiment and price change}
    \label{fig:myimage7}
\end{figure}

\subsection{Sentiment and volatility}
Another experiment compared the aggregated sentiment and the volatility (standard deviation of the last 30d log price change). Here a clear negative correlation could be indentified. If market sentiment is bad, volatility rises. If sentiment is high volatility stabilizes. This is an intersting finding hinting that market mood is linked to price volatility.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{PICs/SentimentVolatilitySameDayCorrelation.png}
    \caption{Linear Sentiment x Volatility correlation}
    \label{fig:myimag8}
\end{figure}
Notable a linear function seem to not fit the data well, because volatility cannot be negative and also it seems like there is a certain threshold rule to be found here.

\subsection{Rolling sentiment}
Similar to the experiment with sentiment and price change the rolling sentiment compares the last 1-30 days rolling sentiment to the next day price change. The goal was to indentify a rolling window that has the strongest spearman correlation and create two histograms of sentiment and price change. If the two histograms can be seperated by a line on the x-axis we could use the value as thershold rule. Here the negative correlation can be found again, indicating that longer time horizons of sentiment does not help overcome the efficient market limitations.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        % Replace with your first filename
        \includegraphics[width=\textwidth]{PICs/RollingWeightedSentimentXPriceChangeCorrelation.png}
        \caption{Pearson \& Spearman correlation}
        \label{fig:image9}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        % Replace with your second filename
        \includegraphics[width=\textwidth]{PICs/RollingSentimentHistogramThreshold.png}
        \caption{Histogram seperation}
        \label{fig:image10}
    \end{subfigure}
    
    % --- Main Caption and Label ---
    \caption{Rolling weighted sentiment and next day price change}
    \label{fig:image9_10}
\end{figure}

As seen in \ref{fig:image10} the seperation of the two histograms is not optimal. As stated by the correlation the histograms also indicate that positive sentiment values lead to negative returns on the next day. Important to note is that adding the impact score did improve the seperation by a small amount (Youden´s J improved from -0.0743 to -0.1068, where 1 or -1 are perfect splits)

\subsection{Better performance with trend prediction}
The experiments with sentiment and weighted sentiment showed a overall negative correlation between the predictor and tomorrow price change. The next approach was to predict a longer time horizon, giving the price returns time to balance out. The idea is that predicting trends would yield more stable and better performance. The new target variables are 3d, 5d and 20d returns.

The results showed that accumulating future returns yield better performance. Especially, the 3d and 5d target gave the ARIMA model with sentiment a greater edge compared to the experiment with 1d returns.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{PICs/3dTargetARMAResults.png}
    \caption{3d Target ARMA Results (Best AIC improved by ~27 points)}
    \label{fig:myimag11}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{PICs/5dTargetARMAResults.png}
    \caption{5d Target ARMA Results (Overall Improvement of the AIC)}
    \label{fig:myimag12}
\end{figure}

The 20d target worked worse than the 1d Target, indicating that today sentiment is not important to predict long time horizons like 20d. If the new targets will also improve non linear models like XGBoost is unknown at this point, and is another experiment in the future.

\subsection{Non Linear Models}
To capture non-linear relationships, an experiment with XGBoost is tasked to classifying the next day return as up or down. Here an improvement in both recall and precision can be achieved by adding weighted sentiment to the model compared to a decision tree with only price change as feature(F1-score 0.5513 improved to 0.6380). If Volatility and sentiment are combined the F1-Score further improves. Howerver the magnitude of this improvement is dependent on the dataset. For example using only LucasPhamFinDataset (like in the other experiments) the increase in model performance is neglegtable (0.0037). If combined with additional headlines from the NIFTY Dataset the improvement is significant (0.0322) which indicate that the quality and amount of headlines matter with the interaction of volatility and weighted sentiment.

Following metrics where measured (LucasPhamFinDataset only):
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{PICs/XGBoostLucasPhamDataset.png}
    \caption{Results of XGBoost on LucasPhamFinDataset}
    \label{fig:myimag13}
\end{figure}

Overall the expermient with XGBoost showed a consistent improvement when adding weighted sentiment to the feature list. A non linear model seems to learn something from the negative correlation.

\section{Milestones and next steps}
\begin{table}[H]
\centering
\caption{Goals Milestone Plan}
\label{tab:milestones}
...
Week 5--6 (Oct) & Use the gathered and prepared data to train ARMA Models and compare the effect of adding the sentiment feature. (first prototype) & Done \\
\hline
Week 7--10 (Oct/Nov) & Create a pipeline that can combine different dataset, gather more data and try around with different sentiment lags (analyse the decay of explainability over time) / compare sentiment models (lexicon -> transformer) & \textbf{Done} \\
\hline
Week 11--12 (Nov) & Create pipeline that calculates technical indicators and combine them with the sentiment scores to create a complete feature Matrix & \textbf{Done} \\
\hline
Week 13--16 (Dec) & Model training and evaluation (ARMA, XGBoost, RNN, LSTM, GPT) Evaluation metrics (MPE, Accuracy, F1-Score) & Pending \\
\hline
Week 17--18 (Jan) & feature contirbution analysis, performance comparision and preparation of presentation & Pending \\
